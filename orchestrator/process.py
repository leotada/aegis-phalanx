...
class ProcessSupervisor:
    """Starts subprocesses in their own process group and stops the whole tree on cancel."""

    def terminate(self, process, force: bool = False) -> None:
    ...
    async def wait_or_cancel(self, process) -> int:
    ...
    async def communicate_or_cancel(self, process) -> tuple[bytes, bytes]:
    ...
    async def run_and_stream(self, command: List[str], cwd: str = "/workspace") -> tuple[int, str, str]:
...
def terminate_process_tree(process, force: bool = False) -> None:
    """Module-level alias so cancellation tests can patch the signal helper."""
    processes.terminate(process, force=force)
