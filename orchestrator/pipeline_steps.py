...
def mode_label(mode: str, *, hard_mode: str) -> str:
    return "Hard (Thorough Review)" if mode == hard_mode else "Easy (Fast)"


def branch_name_from_demand(demand: str) -> str:
    clean_name = re.sub(r'[^a-zA-Z0-9]', '-', demand.lower())[:30].strip('-')
    return f"feature/{clean_name}"


def last_completed_step_name(idx: int, step_name: str, pipeline_config: list[dict]) -> str:
...
async def persist_step(ns, repo_url, demand, step_name, idx, steps_status, git_branch, mode, pipeline_config, status: str) -> None:
    steps_status[step_name] = status
    ns.save_session(
        repo_url,
        demand,
        last_completed_step_name(idx, step_name, pipeline_config) if status != "success" else step_name,
        steps_status,
        git_branch,
        mode=mode,
    )
...
async def execute_pipeline_step(
    ...
    *,
    ...
    try:
        ...
        abort_reason = read_abort_reason(project_dir)
        ...
        await persist_step(
            ns, repo_url, demand, step_name, idx, steps_status, git_branch, mode, pipeline_config, "success"
        )
