...
PIPELINE_CONFIG = [
    ...
    {
        "step_name": "Architect Reviewer (Plan Validation - PLAN)",
        "tool": "agy",
        "model": "gemini-3.1-pro",
        "reasoning_budget": "high",
        "timeout": "5m",
        "prompt": "Read the demand: '{demand}' and the `architect_plan.md` file created by the Architect in the root of the repository. Act as a Principal Software Architect conducting an architectural plan review. Analyze whether the plan makes architectural and technical sense, satisfies the requirements, specifies necessary test cases, and is feasible to implement.\n\nDecision Rules:\n1. If the plan is flawed, unsafe, unrealistic, or fails to address the demand: Create a file named `architect_abort.txt` in the root of the repository containing a clear, detailed explanation of why the plan is rejected and cannot proceed. Do NOT modify `architect_plan.md`.\n2. If the plan is sound and valid: Do NOT rewrite or modify the existing contents of `architect_plan.md`. If you have additional notes, observations, edge cases, or recommendations, append them to the end of `architect_plan.md` in append-only mode under the section `## Architect Review Notes & Observations`. If no additional notes are needed, leave `architect_plan.md` untouched and do NOT create `architect_abort.txt`.",
    },
    {
