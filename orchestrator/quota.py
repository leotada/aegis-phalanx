...
class QuotaMonitor:
    """Captures the agy /usage screen and formats remaining quota for Telegram."""

    def strip_ansi(self, text: str) -> str:
        return re.sub(r"\x1b\[[0-9;?]*[a-zA-Z]", "", text)

    def capture(self, *, timeout: int, scratch_dir: str) -> str:
        ...
        while time.time() < deadline:
            ...
            plain = self.strip_ansi("".join(chunks))
        ...
        return self.strip_ansi("".join(chunks))

    def parse(self, text: str) -> Dict[str, Dict[str, Dict[str, float | str]]]:
        """Parses agy /usage output into remaining and usage percentages per model group."""
        result: Dict[str, Dict[str, Dict[str, float | str]]] = {}
        current_group = None
        current_limit = None

        for line in self.strip_ansi(text).splitlines():
            stripped = line.strip()
    ...
    def format_section(self, quota_data: Dict[str, Dict[str, Dict[str, float | str]]]) -> str:
