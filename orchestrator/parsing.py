...
from orchestrator.urls import RepoUrls, repo_urls


class DemandParser:
    """Extracts a repository, a pipeline mode, and a PR reference from user text."""

    def __init__(self, repos: RepoUrls | None = None):
        self.repos = repos or repo_urls

    def parse(self, demand: str, default_repo: str = None, last_repo: str = None) -> tuple[str, str]:
        """
        ...
        if url_match:
            ...
            return self.repos.normalize(repo_url), clean_demand
        ...
        if shorthand_colon_match:
            ...
            return self.repos.normalize(repo_name), clean_demand
        ...
        if shorthand_exact_match:
            ...
            return self.repos.normalize(repo_name), ""

        # Fallbacks
        if default_repo:
            return self.repos.normalize(default_repo), demand

        if last_repo:
            return self.repos.normalize(last_repo), demand

        return None, demand

    def extract_mode(self, text: str) -> tuple[str, str]:
        """
    ...
    def parse_pr_reference(self, text: str, default_repo: str = None) -> tuple[str | None, int | None]:
        """
        ...
        if url_match:
            return self.repos.normalize(url_match.group(1)), int(url_match.group(2))
        ...
        if hash_match:
            return self.repos.normalize(hash_match.group(1)), int(hash_match.group(2))
        ...
        if sep_match:
            return self.repos.normalize(sep_match.group(1)), int(sep_match.group(2))

        num_match = re.match(r"^#?(\d+)$", cleaned)
        if num_match and default_repo:
            return self.repos.normalize(default_repo), int(num_match.group(1))
