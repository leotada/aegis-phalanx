...
class GithubAuthenticator:
    """Picks an authenticated clone URL: env token, then gh CLI, then SSH."""
    ...
    async def resolve_clone_url(
        ...
        *,
        ...
        get_gh_token = gh_token_fn or self.token
        gh_token = await get_gh_token()
        if gh_token:
            return _with_token(gh_token), "gh-cli", None

        # 3. SSH key fallback.
        check_ssh = ssh_available_fn or self.ssh_available
        owner_repo = self.repos.owner_repo(repo_url)
